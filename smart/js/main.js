var HomePage = function () {
  ...
  var _initCalculation = function () {
    ...
    var _calculateTotal = function (money, percent, period) {
      ...
      return [t.toLocaleString('en-US'), n.toLocaleString('en-US'), (t - n).toLocaleString('en-US')];
    };

    var _appendTotalValueArea = function () {
      // 有變更即馬上試算
      var results = _calculateTotal($('#sliderInput1').val(), $('#sliderInput2').val(), $('#sliderInput3').val());
    ...
    $('input.range-slider__value').on('input', function () {
      console.log($(this).val());
      var value = $(this).val();
      var $slider = $(this).closest('.range-slider').find('input.range-slider__range');
      $slider.val(value);
  ...
  var _activeTab = function (index) {
    ...
    tabContent.steps.forEach((e, index) => {
      $('.number_step').append(`<li class="${index === 0 ? 'active_number' : 'normal_number'}">0${index + 1}</li>`);
    });
    ...
    $('.circle-arrow')
      ...
      .on('click', function () {
        ...
        else if (nextIndex > tabContent.steps.length - 1) nextIndex = tabContent.steps.length - 1;
    ...
    _timer = setInterval(function () {
      var currentStepIndex = $('.number_step .active_number').length - 1;
      var nextIndex = currentStepIndex + 1 > tabContent.steps.length - 1 ? 0 : currentStepIndex + 1;
      _activeStep(nextIndex);
    }, 3300);
  };
  ...
  var _openSubmenu = function () {
    ...
    if ($('.header_item .border_top').length <= 0) $('<div class="border_top"></div>').insertBefore('.pc_menu');
  };
